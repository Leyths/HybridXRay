...
bool EFS_Utils::GetOpenNameEx(HWND hWnd, LPCSTR initial, xr_string& buffer, const GUID& client_guid, bool bMulti, LPCSTR offset, int start_flt_ext)
{
    char buf[255 * 255];   // max files to select
    xr_strcpy(buf, buffer.c_str());

    bool bRes = GetOpenNameInternal(hWnd, initial, buf, sizeof(buf), bMulti, offset, start_flt_ext, &client_guid);
